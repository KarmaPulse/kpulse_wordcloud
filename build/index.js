...
var WordCloud = function (_React$Component) {
  ...
  _createClass(WordCloud, [{
    ...
    value: function _update(props) {
      ...
      this._vis.attr('transform', 'translate(' + this._width / 2 + ', ' + this._height / 2 + ')scale(1.0020877122879028,1.0020877122879028)');
      ...
      this.fontSizeNew = function (value) {
        var initialValues = {
          MAX: _lodash2.default.maxBy(words, 'total'),
          MIN: _lodash2.default.minBy(words, 'total'),
          FONT_MAX: 40,
          FONT_MIN: 12
        };
      ...
      this._layout.size([this._width, this._height]).words(filteredWords).padding(1).text(this._setText).font(fontFamily).fontSize(function (d) {
        return _this3.fontSizeNew(d[wordCountKey]);
